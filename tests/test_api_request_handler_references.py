...
class TestApiRequestHandlerReferences:
    ...
    def test_add_case_references_success(self, references_handler):
        ...
        mock_update_response = APIClientResult(
            status_code=200, response_text={"id": 1, "refs": "REQ-1, REQ-2, REQ-3, REQ-4"}, error_message=None
        )

        with patch.object(references_handler.client, "send_get", return_value=mock_get_case_response), patch.object(
            references_handler.client, "send_post", return_value=mock_update_response
        ):

            success, error = references_handler.add_case_references(case_id=1, references=["REQ-3", "REQ-4"])
            ...
            references_handler.client.send_post.assert_called_once_with(
                "update_case/1", {"refs": "REQ-1,REQ-2,REQ-3,REQ-4", "is_legacy": True}
            )
    ...
    def test_add_case_references_no_existing_refs(self, references_handler):
        ...
        # Mock update_case response
        mock_update_response = APIClientResult(
            status_code=200, response_text={"id": 1, "refs": "REQ-1, REQ-2"}, error_message=None
        )
        ...
        with patch.object(references_handler.client, "send_get", return_value=mock_get_case_response), patch.object(
            ...
            success, error = references_handler.add_case_references(case_id=1, references=["REQ-1", "REQ-2"])
            ...
            references_handler.client.send_post.assert_called_once_with(
                "update_case/1", {"refs": "REQ-1,REQ-2", "is_legacy": True}
            )
    ...
    def test_add_case_references_avoid_duplicates(self, references_handler):
        ...
        with patch.object(references_handler.client, "send_get", return_value=mock_get_case_response), patch.object(
            ...
            success, error = references_handler.add_case_references(
                case_id=1, references=["REQ-1", "REQ-3"]  # REQ-1 already exists
            )
            ...
            references_handler.client.send_post.assert_called_once_with(
                "update_case/1", {"refs": "REQ-1,REQ-2,REQ-3", "is_legacy": True}
            )

    def test_add_case_references_case_not_found(self, references_handler):
        """Test handling of non-existent test case"""
        mock_get_case_response = APIClientResult(
            status_code=404, response_text=None, error_message="Test case not found"
        )

        with patch.object(references_handler.client, "send_get", return_value=mock_get_case_response):

            success, error = references_handler.add_case_references(case_id=999, references=["REQ-1"])
    ...
    def test_add_case_references_character_limit_exceeded(self, references_handler):
        ...
        mock_get_case_response = APIClientResult(
            status_code=200,
            response_text={"id": 1, "title": "Test Case 1", "refs": "REQ-1" * 500},  # Long existing refs
            error_message=None,
        )
        ...
        with patch.object(references_handler.client, "send_get", return_value=mock_get_case_response):
            ...
            success, error = references_handler.add_case_references(case_id=1, references=long_refs)
    ...
    def test_add_case_references_deduplication(self, references_handler):
        ...
        with patch.object(references_handler.client, "send_get", return_value=mock_get_case_response), patch.object(
            ...
            success, error = references_handler.add_case_references(
                case_id=1, references=["REQ-2", "REQ-2", "REQ-3", "REQ-2"]  # Duplicates should be removed
            )
            ...
            references_handler.client.send_post.assert_called_once_with(
                "update_case/1", {"refs": "REQ-1,REQ-2,REQ-3", "is_legacy": True}  # Duplicates removed, order preserved
            )
    ...
    def test_update_case_references_success(self, references_handler):
        ...
        with patch.object(references_handler.client, "send_post", return_value=mock_update_response):
            ...
            success, error = references_handler.update_case_references(case_id=1, references=["REQ-3", "REQ-4"])
            ...
            # Verify the API call
            references_handler.client.send_post.assert_called_once_with(
                "update_case/1", {"refs": "REQ-3,REQ-4", "is_legacy": True}
            )
    ...
    def test_update_case_references_character_limit_exceeded(self, references_handler):
        ...
        success, error = references_handler.update_case_references(case_id=1, references=long_refs)
    ...
    def test_update_case_references_deduplication(self, references_handler):
        ...
        with patch.object(references_handler.client, "send_post", return_value=mock_update_response):
            ...
            success, error = references_handler.update_case_references(
                case_id=1, references=["REQ-1", "REQ-1", "REQ-2", "REQ-1"]  # Duplicates should be removed
            )
            ...
            # Verify the API call has deduplicated references
            references_handler.client.send_post.assert_called_once_with(
                "update_case/1", {"refs": "REQ-1,REQ-2", "is_legacy": True}  # Duplicates removed, order preserved
            )
    ...
    def test_update_case_references_api_failure(self, references_handler):
        ...
        mock_update_response = APIClientResult(
            status_code=400, response_text=None, error_message="Invalid test case ID"
        )

        with patch.object(references_handler.client, "send_post", return_value=mock_update_response):

            success, error = references_handler.update_case_references(case_id=1, references=["REQ-1"])
    ...
    def test_delete_case_references_all_success(self, references_handler):
        ...
        # Mock update_case response
        mock_update_response = APIClientResult(status_code=200, response_text={"id": 1, "refs": ""}, error_message=None)

        with patch.object(references_handler.client, "send_post", return_value=mock_update_response):

            success, error = references_handler.delete_case_references(
                case_id=1, specific_references=None  # Delete all
            )
            ...
            # Verify the API call
            references_handler.client.send_post.assert_called_once_with(
                "update_case/1", {"refs": "", "is_legacy": True}
            )
    ...
    def test_delete_case_references_specific_success(self, references_handler):
        ...
        mock_get_case_response = APIClientResult(
            status_code=200,
            response_text={"id": 1, "title": "Test Case 1", "refs": "REQ-1, REQ-2, REQ-3, REQ-4"},
            error_message=None,
        )

        # Mock update_case response
        mock_update_response = APIClientResult(
            status_code=200, response_text={"id": 1, "refs": "REQ-1, REQ-4"}, error_message=None
        )
        ...
        with patch.object(references_handler.client, "send_get", return_value=mock_get_case_response), patch.object(
            ...
            success, error = references_handler.delete_case_references(
                case_id=1, specific_references=["REQ-2", "REQ-3"]
            )
            ...
            references_handler.client.send_get.assert_called_once_with("get_case/1")
            references_handler.client.send_post.assert_called_once_with(
                "update_case/1", {"refs": "REQ-1,REQ-4", "is_legacy": True}
            )
    ...
    def test_delete_case_references_no_existing_refs(self, references_handler):
        ...
        with patch.object(references_handler.client, "send_get", return_value=mock_get_case_response):
            ...
            success, error = references_handler.delete_case_references(case_id=1, specific_references=["REQ-1"])
    ...
    def test_delete_case_references_case_not_found(self, references_handler):
        ...
        with patch.object(references_handler.client, "send_get", return_value=mock_get_case_response):
            ...
            success, error = references_handler.delete_case_references(case_id=999, specific_references=["REQ-1"])
