...
class TestCmdParseJunitValidation:
    ...
    def test_handle_case_update_reporting_console(self):
        ...
        case_update_results = {
            ...
            "failed_cases": [{"case_id": 789, "case_title": "Test Case 3", "error": "API error"}],
        }
        ...
        assert "Case Updates Summary:" in output
        assert "Updated cases: 1" in output
        assert "Skipped cases: 1" in output
        assert "Failed cases: 1" in output

    def test_handle_case_update_reporting_with_fields(self):
        """Test _handle_case_update_reporting console output with field updates"""
        env = Mock()
        env.json_output = False
            ...
            "updated_cases": [
                {
                    ...
                    "added_refs": [],
                    "skipped_refs": [],
                    "updated_fields": ["custom_preconds", "custom_steps", "custom_expected"],
                }
            ],
            "skipped_cases": [],
            "failed_cases": [],
        ...
        # Verify console output includes field updates
        assert env.log.called
        call_args = [call[0][0] for call in env.log.call_args_list]
        output = " ".join(call_args)
        assert "Case Updates Summary:" in output
        assert "Updated cases: 1" in output
        assert "updated 3 field(s): custom_preconds, custom_steps, custom_expected" in output

    def test_handle_case_update_reporting_with_refs_and_fields(self):
        """Test _handle_case_update_reporting with both references and fields"""
        env = Mock()
        env.json_output = False
            ...
            "updated_cases": [
                {
                    "case_id": 456,
                    "case_title": "Test Case 2",
                    "added_refs": ["REF-1", "REF-2"],
                    "skipped_refs": ["REF-3"],
                    "updated_fields": ["custom_automation_type"],
                }
            ],
            "skipped_cases": [],
            "failed_cases": [],
        ...
        # Verify both refs and fields are shown
        assert env.log.called
        call_args = [call[0][0] for call in env.log.call_args_list]
        output = " ".join(call_args)
        assert "added 2 refs, skipped 1 duplicates" in output
        assert "updated 1 field(s): custom_automation_type" in output
        ...
        for i, (success, message, added_refs, skipped_refs) in enumerate(api_responses):
            ...
            if not success:
                ...
                skipped_cases.append(
                    {"case_id": case_id, "reason": "All references already present", "skipped_refs": skipped_refs}
                )
            else:
                updated_cases.append({"case_id": case_id, "added_refs": added_refs, "skipped_refs": skipped_refs})
