...
def cli(environment: Environment, context: click.Context, *args, **kwargs):
    ...
    if not environment.password and not environment.key:
        ...
        sys.exit(1)
    if not environment.plan_id:
        click.echo("Please provide a plan ID using the --plan-id argument.", err=True)
        sys.exit(1)
    ...
    minimal_suite = TestRailSuite(name="Plan Query", testsections=[])
    ...
    data, error_message = api_request_handler.get_plan(environment.plan_id)
    if error_message:
