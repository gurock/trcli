...
@click.option(
    "--auto-close-run", is_flag=True, default=False, help="Use this option to automatically close the created run."
)
@click.option(
    "--run-case-ids",
    metavar="",
...
@click.option("--run-refs", metavar="", help="A comma-separated list of references/requirements (up to 250 characters)")
@click.option(
    "--run-refs-action",
...
def cli(environment: Environment, context: click.Context, *args, **kwargs):
    ...
    # Validation: mutually exclusive assignee flags
    if environment.run_assigned_to_id and environment.clear_run_assigned_to_id:
        environment.elog("Error: --run-assigned-to-id and --clear-run-assigned-to-id cannot be used together.")
        exit(1)

    # Validation: clear flag requires --run-id (update mode only)
    if environment.clear_run_assigned_to_id and not environment.run_id:
        environment.elog(
            "Error: --clear-run-assigned-to-id can only be used when updating an existing run (--run-id required)."
        )
        exit(1)
